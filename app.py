...
for i, row in df_editado.iterrows():
    ...
    if sector.lower() not in ['none', 'nan', '', '<na>']:
        ...
        if i < len(df_editado) - 1 and "Continúa" in continua:
            ...
            # Corrección aplicada aquí para que tome tanto "dirección" como "secretaría"
            if "otra" in continua.lower() and deriva.lower() not in ['', 'nan', 'none']:
                label_flecha += f"\n(Hacia {deriva})"
        ...
        if st.button("🚀 Guardar en Google Sheets", use_container_width=True, type="primary"):
            if not nombre_tramite or not sectores_cargados:
                ...
                try:
                    ...
                    # Corrección de asignación de metadata
                    ...
                    columnas_finales = [
                        ...
                        "Posee_Certificacion", "Nombre_Certificado", "Destinatario_Final"
                    ]
